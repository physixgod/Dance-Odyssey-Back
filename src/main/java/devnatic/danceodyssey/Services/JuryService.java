...
public  List<Event> getAllEvents() {
    ...
    @Override
    public Workshop addWorkshop(Workshop workshop, int idGroup) {
        Optional<Group> optionalGroup = groupRepository.findById(idGroup);
        if (optionalGroup.isPresent()) {
            Group group = optionalGroup.get();
            Set<Dancer> groupDancers = group.getDancers();
            Workshop savedWorkshop = workshopRepository.save(workshop);

            // Prepare email content
            String emailSubject = "New Workshop Created: " + workshop.getName();
            String emailText = "Dear Dancer,<br><br>"
                    + "A new workshop titled '<strong>" + workshop.getName() + "</strong>' has been created in your group.<br><br>"
                    + "<strong>Details:</strong><br>"
                    + "<ul>"
                    + "<li><strong>Description:</strong> " + workshop.getDescription() + "</li>"
                    + "<li><strong>Date:</strong> " + workshop.getDate().toString() + "</li>"
                    + "<li><strong>Location:</strong> " + workshop.getLocation() + "</li>"
                    + "</ul>"
                    + "Please check your group for more information.<br><br>"
                    + "Best regards,<br>"
                    + "Dance Odyssey Team";

            // Send email notification to each dancer
            for (Dancer dancer : groupDancers) {
                String dancerEmail = dancer.getEmail();
                try {
                    emailSender.sendSimpleMessage(dancerEmail, emailSubject, emailText);
                } catch (MessagingException e) {
            ...
            // Handle case where group with the provided ID does not exist
            throw new NoSuchElementException("Group with ID " + idGroup + " not found");
        }
    }
    ...
    public String dancerRegisterAtEvent(int idDancer, int idEvent) {
        Dancer dancer = dancerRepsoitory.findById(idDancer).orElse(null);
